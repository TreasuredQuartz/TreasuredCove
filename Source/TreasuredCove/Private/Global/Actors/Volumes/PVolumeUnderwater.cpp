...
#include "Global/Actors/Volumes/PVolumeUnderwater.h"

#include "Components/BrushComponent.h"
#include "PhysicsEngine/PhysicsSettings.h"
#include "EngineUtils.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PVolumeUnderwater)

APVolumeUnderwater::APVolumeUnderwater(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	// this physics volume is considered water
	bWaterVolume = true;
...
void APVolumeUnderwater::PostUnregisterAllComponents(void)
{
...
void APVolumeUnderwater::PostRegisterAllComponents()
{
	Super::PostRegisterAllComponents();
	GetWorld()->InsertPostProcessVolume(this);
}

bool APVolumeUnderwater::IsPPVEnabled() const
{
#if WITH_EDITOR
...
bool APVolumeUnderwater::EncompassesPoint(FVector Point, float SphereRadius/*=0.f*/, float* OutDistanceToPoint)
{
...
void APVolumeUnderwater::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
#if WITH_EDITOR
	if (Ar.IsLoading())
...
void APVolumeUnderwater::ActorEnteredVolume(AActor* Other)
{
...
void APVolumeUnderwater::ActorLeavingVolume(AActor* Other)
{
...
#if WITH_EDITOR
void APVolumeUnderwater::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (ChangedPropertyName == GET_MEMBER_NAME_CHECKED(APVolumeUnderwater, bUnbound))
	{
		if (bUnbound)
		{
...
bool APVolumeUnderwater::CanEditChange(const FProperty* InProperty) const
{
	if (InProperty)
	{
		...
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		{
			...
			if (PropertyName == GET_MEMBER_NAME_STRING_CHECKED(APVolumeUnderwater, bEnabled))
			{
				return true;
			}
		...
		if (PropertyName == GET_MEMBER_NAME_STRING_CHECKED(APVolumeUnderwater, BlendRadius))
		{
			if (bUnbound)
			{
				return false;
