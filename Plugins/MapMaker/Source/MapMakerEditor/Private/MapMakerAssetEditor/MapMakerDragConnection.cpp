...
TSharedRef<FMapMakerDragConnection> FMapMakerDragConnection::New(const TSharedRef<SGraphPanel>& GraphPanel, const FDraggedPinTable& DraggedPins)
{
	TSharedRef<FMapMakerDragConnection> Operation = MakeShareable(new FMapMakerDragConnection(GraphPanel, DraggedPins));
	Operation->Construct();

	return Operation;
}

void FMapMakerDragConnection::OnDrop(bool bDropWasHandled, const FPointerEvent& MouseEvent)
{
...
void FMapMakerDragConnection::OnDragged(const class FDragDropEvent& DragDropEvent)
{
...
void FMapMakerDragConnection::HoverTargetChanged()
{
	...
	else if (UEdNode_MapMakerNode* TargetNodeObj = Cast<UEdNode_MapMakerNode>(GetHoveredNode()))
	{
		TArray<UEdGraphPin*> ValidSourcePins;
		ValidateGraphPinList(/*out*/ ValidSourcePins);
...
#define LOCTEXT_NAMESPACE "AssetSchema_MapMaker"
				Response = FPinConnectionResponse(CONNECT_RESPONSE_DISALLOW, LOCTEXT("PinError", "Not a valid UMapMakerEdNode"));
...
FMapMakerDragConnection::FMapMakerDragConnection(const TSharedRef<SGraphPanel>& GraphPanelIn, const FDraggedPinTable& DraggedPinsIn)
...
FReply FMapMakerDragConnection::DroppedOnPin(FVector2D ScreenPosition, FVector2D GraphPosition)
{
	TArray<UEdGraphPin*> ValidSourcePins;
	ValidateGraphPinList(/*out*/ ValidSourcePins);
...
FReply FMapMakerDragConnection::DroppedOnNode(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
FReply FMapMakerDragConnection::DroppedOnPanel(const TSharedRef< SWidget >& Panel, FVector2D ScreenPosition, FVector2D GraphPosition, UEdGraph& Graph)
{
	// Gather any source drag pins
...
void FMapMakerDragConnection::ValidateGraphPinList(TArray<UEdGraphPin*>& OutValidPins)
{
