...
#include "GenericSubobjectData.generated.h"
...
struct GENERICGRAPHEDITOR_API FGenericSubobjectData final
{
...
public:
	...
	*/
	 ...
	 */
	inline const UGenericGraphNodeComponent* GetComponentTemplate(bool bEvenIfPendingKill = false) const
	{
		return Cast<UGenericGraphNodeComponent>(WeakObjectPtr.Get(bEvenIfPendingKill));
	}

	inline const UGenericGraphNodeComponent* FindComponentInstanceInNode(const UGenericGraphNode* InNode) const { return FindMutableComponentInstanceInNode(InNode); }
	...
	bool IsAttachedTo(const FGenericSubobjectDataHandle& InHandle) const;
	...
	void SetupAttachment(FName SocketName, const FGenericSubobjectDataHandle& AttachParentHandle = FGenericSubobjectDataHandle::InvalidHandle);

	FGenericSubobjectDataHandle GetHandle() const { return Handle; }
	FGenericSubobjectDataHandle GetParentHandle() const { return ParentObjectHandle; }
	FGenericSubobjectData* GetParentData() const { return ParentObjectHandle.GetData(); }
	bool HasParent() const { return ParentObjectHandle.IsValid(); }
	const TArray<FGenericSubobjectDataHandle>& GetChildrenHandles() const { return ChildrenHandles; }

	/** Walks the parent heirarchy and returns the root subobject in this case */
	FGenericSubobjectDataHandle GetRootSubobject() const;

	// Return true if the given handle is a child of this subobject
	bool HasChild(const FGenericSubobjectDataHandle& ChildHandle) const;

	// Attempt to find the given handle in out child array and return a pointer to it
	FGenericSubobjectDataHandle FindChild(const FGenericSubobjectDataHandle& ChildHandle) const;
	FGenericSubobjectDataHandle FindChildByObject(UObject* ContextObject) const;
...
private:
	...
	*/
	bool AddChildHandleOnly(const FGenericSubobjectDataHandle& InHandle);
	...
	* Actual manipulation of subobjects should be done via the SubobjectSubsystem.
	*/
	bool RemoveChildHandleOnly(const FGenericSubobjectDataHandle& InHandle);

	void SetParentHandle(const FGenericSubobjectDataHandle& NewParentHandle) { ParentObjectHandle = NewParentHandle; }
	void ClearParentHandle() { ParentObjectHandle = FGenericSubobjectDataHandle::InvalidHandle; }
	...
	UGenericGraphNodeComponent* GetMutableNodeContext();
	...
	inline UGenericGraphNodeComponent* GetMutableComponentTemplate(bool bEvenIfPendingKill = false) const
	{
		return Cast<UGenericGraphNodeComponent>(WeakObjectPtr.Get(bEvenIfPendingKill));
	}

	UGenericGraphNodeComponent* FindMutableComponentInstanceInNode(const UGenericGraphNode* InNode) const;
